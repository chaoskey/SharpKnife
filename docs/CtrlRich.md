...
>
> 20211221, 在任意地方选择文本然后Ctrl-cf翻译（需要谷歌浏览器和划词翻译扩展支持）
>
...
> 建议配置成开机启动
>
> 为了可在任意环境下运行，建议以管理员身份运行。（比如 Ctrl-cf是选择翻译，如果是非管理员状态，有些环境会触发搜索快捷键Ctrl-f；用管理员身份运行可以避免这个问题: Ctrl-cf不会触发Ctrl-f搜索，只有Ctrl-f才能触发搜索）
...
我现在希望增强RCtrl功能（用此脚本的情况），我约定`RCtrl-[cvxsdafewt]+`, 如果右Ctrl按下没松开不会触发，字母按下会等待松开(RCtrl按下的前提下)，最后RCtrl松开才会触发。 

说人话就是： Ctrl按下后必须松开才触发，每个字符必须是一次完整的点击(按下-松开)。 

`[cvxsdafewt]+`使用正则表达式描述的，意思是：`[cvxsdafewt]`代表一个特定字符(`cvxsdafewt`之一), 而`+`代表至少一个字符点击。

这说明，只有`RCtrl-[cvxsdafewt]+`相关的Ctrl功能被增强，除此之外没有任何影响或改变。

之所以增强RCtrl功能，是因为我和大部分都是“右撇子”；同样道理，这些特殊字符`cvxsdafewt`选用是因为它们都在键盘左边（如图）。 “一左一右”符合快捷操作的习惯。
...
- `RCtrl-[cvx]`     系统复制粘贴剪切 (复制和剪切的内容会被作为历史记录)

- `RCtrl-cc`        截图 (需要Snipaste支持，截图的内容会被作为历史记录)

- `RCtrl-vv`        贴图 (需要Snipaste支持)

- `RCtrl-cv`        截贴图 (需要Snipaste支持，截图的内容会被作为历史记录)

- `RCtrl-sv`        搜索粘贴 (搜索剪切板历史记录)

- `RCtrl-ce`        剪切板编辑 (目前只支持文本剪切板编辑)

- `RCtrl-ww`        屏幕白板 (需要Snipaste支持)

- `RCtrl-ct`        标签修改

- `RCtrl-cf`        选择翻译 (需要谷歌浏览器和划词翻译插件支持)

- `RCtrl-ff`        弹出翻译框 (需要谷歌浏览器和划词翻译插件支持)
...
- 3）安装了“划词翻译”（chrome 网上应用店搜索“划词翻译”）

- 4）chrome://extensions/shortcuts，启用“打开独立翻译窗口”快捷键: Ctrl+Shift+1

- 5) 确保使用此功能前，曾经打开过此独立翻译窗口。

独立翻译窗口一旦打开，此扩展作为Chrome后台应用始终存在，即使谷歌浏览器已经关闭。

如果没有此脚本的支持，一旦谷歌浏览器关闭，并且独立翻译窗口，那么必须先启动谷歌浏览器。

- 1) **选择翻译** 【功能命令： `RCtrl-cf`】

**选择翻译**的程序流程是: 手工选择-复制(c)-翻译(f) ，所以命令取 “cf”

如果没有手工选择，则无动作。

- 2) **弹出翻译框** 【功能命令： `RCtrl-ff`】
