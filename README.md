...
| 步骤 | 判定 | 走的通道 / 允许的动作 |
|---|---|---|
| 第一步 | **要输出文字**（要网址、要答案、要一段内容、翻译、解释…） | 直接走文字通道：`paste: 文本`（**推荐**，任何符号都原样粘贴）或 `send: 文本`（文本里不能含 `^` `!` `+` `#` `{` `}`）；**不需要预先配置** |
...
- **被丢弃的行会显示在清单里**（例如模型多写的解释文字、没加 `send:` / `paste:` 的裸文字、表里没有的 `run:`），一眼就能看出它有没有乱来；**只有显式写了 `send:` / `paste:` 的文字才会被输出**，模型的口水话不会打进您的编辑器。
