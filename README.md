...
**动作词汇与配置完全一致**（模型只能在这些里面挑，其它一行都不执行）：

| 类别 | 允许的动作 | 说明 |
|---|---|---|
| 自由内容 | `send: 内容` / `hotkey: 内容` | 按键、组合键、要输入的文本 |
| 自由内容 | `paste: 文本` | 剪贴板粘贴，长文本优先用它 |
| **必须命中配置** | `run: 命令行` / `self: 命令` | 命令行 / 自身功能**必须与您配置里某一项完全一样**，否则丢弃 |
| **必须命中配置** | `item: 名称` | 按名字执行您配好的那一项（最稳，模型优先用这个） |

- 解析前程序会把您的**「已配置命令表」**（四块小键盘每个键 + 圆盘菜单每一项的"名字 = 动作"）整份喂给模型，并要求它**优先复用**；模型不会凭空发明您没配过的程序与功能。
- **被丢弃的行会显示在清单里**（例如模型多写的解释文字、表里没有的 `run:`），一眼就能看出它有没有乱来；绝不会把模型的口水话打进您的编辑器。
...
- **执行完焦点会还给原来的窗口**：动作跑完后，焦点回到"最近一次活动的窗口"（也就是动作执行的目标窗口），运行框留在屏幕上但**不再抢焦点**，您可以接着在原窗口继续干活；想回来下一条需求，**点一下输入框**或**再按一次触发键**即可（触发键此时只会把焦点拿回来，不会关掉窗口；只有焦点已经在运行框里时，触发键才是"关闭"）。
- **执行完自动回到输入状态**：跑完（或被 `Esc` 中止）后，刚执行的动作会留在清单里可回看，输入框自动清空并重新聚焦，**可以直接接着输入下一条需求**，不用再按一次热键；想关掉按 `Esc` 或再按触发键。
...
- **只执行"配置里已有的动作"（重要）**：模型不能自造按键、程序或功能名，也**不能输出自由文字**。中文需求会被翻译成"圆盘菜单 / 四块小键盘里已配置的动作"组成的一串动作，然后依次执行；任何不在配置里的行都会被**拦下并列出原因**，绝不执行。
  **名称与动作都区分大小写**（`item: A` 与 `item: a` 是两个不同的项，分别对应配置里的 A 与 a）；字母键盘（`case = true`）的**大小写两种字母都会列进动作表**，所以"说出大写字母"用 `item: A` 即可。
  所以想让运行框能做某件事，先在 `config.ini` 的小键盘 / 圆盘菜单里建一项（`编号 = 名称 | 动作`），模型就能用 `item: 名称` 引用它。
  **要输入一段文字**（例如"写上你好"）：在小键盘或圆盘菜单里加一项，动作写 `paste: 你好`（或 `send: 你好`），运行框里说"写上你好"就会命中这一项。
