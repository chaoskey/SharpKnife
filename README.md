...
- **运行框正下方有一排热键键帽**：回车、Tab、空格、删除、退格、取消、触发。点一下就把这个键**发给目标窗口**（键帽自己不抢焦点，所以不会打断您正在编辑的窗口）；「取消」发送的是 Esc，「触发」等同于本脚本的补全功能（`self:trigger`）。
...
- **执行完焦点归谁（看有没有打开新窗口）**：① 动作**打开了新窗口**（例如 `run:` 启动了程序、或打开了 Windows 自带的运行框）→ **焦点就留在这个新窗口里**，运行框和原来那个窗口都不抢；② **没有打开新窗口** → 焦点回到执行前那个"最近一次活动的窗口"（例如您原来在编辑的文本框），可以接着打字。判定时**一律把自然语言运行框自己排除在外**：它既可以有焦点（您在里面打字），但永远不算动作的目标窗口，也不算"要回退到的上一个窗口"。无论哪种情况，运行框都留在屏幕上、**不抢焦点**；想回来下一条需求，**点一下输入框**或**再按一次触发键**即可（触发键此时只会把焦点拿回来，不会关掉窗口；只有焦点已经在运行框里时，触发键才是"关闭"）。
- **执行完自动回到输入状态**：跑完（或被 `Esc` 中止）后，刚执行的动作会留在清单里可回看，输入框自动清空（焦点去向见上一条），可以直接接着输入下一条需求；想关掉按 `Esc` 或再按触发键。
...
hotkey = ^+i          ; 触发键（默认 Ctrl+Shift+I）；再按一次关闭运行框
