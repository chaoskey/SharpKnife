...
**配置格式**（`config.ini` 的 `[radial]` 段，详见「配置说明」）：每个功能组一个子节 `[radial.xxx]`（`xxx` 为英文标识符，不显示），组显示名由 `name` 指定（中文）；组内功能以编号 `1`/`2`/`3`... 为 key，值格式为 `中文功能名 | 动作`。这里的“动作”支持两种写法：`快捷键`，或 `run: 完整命令行`。**组的出现顺序 = 节在文件中的行序；编号决定功能排列顺序**。
...
- **字母小键盘**（默认 `Ctrl+Shift+E`）：6 列 × 5 行共 **30 键**——`a`–`z` 顺序排列，末尾四个键分别是大小写切换、回车、反斜杠和【触发】：
  ...
  y  z  Aa  回车  \  触发
  ```
  - 点【Aa】一次，整块面板在大写 / 小写之间切换（字母标签与发送内容一起变；大写时【Aa】键变成暖色，相当于 CapsLock 指示灯）；状态在本次运行期间一直记住。小写发送 `a`，大写发送 `A`。
  - 【回车】发送 `{Enter}`，反斜杠键发送 `\`（与符号面板里的 `\` 重复，是应要求加的）。
  - 【触发】**等同于按 SharpKnife 的触发命令 `Ctrl+J`**（即"补全"入口；该组合键在 `config.ini` 的 `[trigger] hotkey` 里可改）。它是直接调用补全逻辑，不是模拟按键，所以在任何输入焦点下都可靠。
...
opacity = 1            ; 面板本体透明度 0.0~1.0（0=完全透明，1=不透明；缺省 1）；文字始终红色且不透明，不受此项影响

[radial]               ; —— 径向菜单（快捷菜单）——
trigger = ^+m         ; 触发径向菜单的全局快捷键（默认 Ctrl+Shift+M）
font_size = 15        ; 径向菜单字体大小（磅，最小 6；缺省/删除本行 = 沿用 [ui] font_size）
opacity = 1          ; 菜单本体透明度 0.0~1.0（0=完全透明，1=不透明；缺省 1）；文字始终红色且不透明，不受此项影响
...
;            VSCode 打开配置 | run: "C:\Users\joistw\AppData\Local\Programs\Microsoft VS Code\Code.exe" "E:\Working\SharpKnife\config.ini"
[radial.base]
