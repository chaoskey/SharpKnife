...
屏幕小键盘触发  →  面板未打开 → 以鼠标位置为中心弹出面板（方向键盘 / 数字键盘 / 符号键盘）
...
| 符号小键盘 | `Ctrl+Shift+Y`（弹出 27 个常用符号 6 列 × 5 行 小键盘；再按同一键关闭） | `[keypad] symbol_hotkey` |
...
>
> **关闭方式**：触发键（第二次按下）、`Esc` 键，或鼠标右键（任意层）；也可用第一层圆心【常用】关闭。圆盘与三个屏幕小键盘**互相独立、可同时显示**；四者共用 `Esc` 时，`Esc` 关闭的是**你最近操作过的那个**浮层（鼠标移入、点击——含点空位/无效点击——都算一次操作）。四者同屏时**互不重叠**（拖动会推开彼此），详见下文「屏幕小键盘」一节。
...
三个**独立的全局快捷键入口**（默认 `Ctrl+Shift+K`、`Ctrl+Shift+N` 与 `Ctrl+Shift+Y`，见 `config.ini` 的 `[keypad]` 段），以鼠标位置为中心弹出**屏幕按键面板**，面向数位板 / 触屏场景：用笔点按键，该按键就发送到**当前前台窗口**。
...
- **符号小键盘**（默认 `Ctrl+Shift+Y`）：6 列 × 5 行共 **27 个常用符号**，是标准键盘里**除方向 / 数字面板已有键之外**的其余符号：
  ```
  (  )  [  ]  {  }
  <  >  \  |  ;  :
  '  "  ,  ?  !  @
  ...
  =  ~  `
  ```
  与字符键一样发送**普通字符**（`^` `!` `#` `{` `}` 等已按 `Send` 规则转义，直接可用；不受 NumLock 影响）。

> **开/关切换**：每个触发键**只管自己的面板**——未打开时弹出、已打开时再按则关闭；三块面板**可以同时打开**，互不影响。
>
...
>
> **四个浮层互相独立**：方向小键盘、数字小键盘、符号小键盘、径向菜单**可以同时显示**——打开或关闭其中一个都不会联动关闭另外几个。四者共用 `Esc`：按一次 `Esc` 关闭**你最近操作过的那个**浮层（打开面板、鼠标移到面板上、在面板上点击——**包括点空位 / 无效点击、拖动**都算一次操作），全部关完后 `Esc` 立即归还给系统。
>
> **互不重叠（自动避让）**：四块浮层同屏时**不允许压在一起**——拖动其中一块压到别的块时，被压住的块会自动让开（沿"位移最小"的方向推开，两两之间留 8 像素间隙；被推的块压到第三块时继续连锁让开；所有落点都限制在屏幕可见区域内）。**新打开的面板**同样以它为准，把被它压住的旧面板推开；屏幕边上实在推不开时保留最小重叠，不会来回抖动。径向菜单、方向 / 数字 / 符号小键盘都适用。
>
> **关闭方式**：自己的触发键（再按一次）、`Esc`、鼠标右键（三套面板都没有【关】键——方向小键盘的中心已改为【回车】）。
>
...
symbol_hotkey = ^+y    ; 符号小键盘触发键（默认 Ctrl+Shift+Y；6 列 × 5 行共 27 个常用符号，见下文）
...
- **屏幕小键盘**：`KeypadLoadConfig()` 用 `IniRead` 读取 `[keypad]`（防空 + 防呆）；`KeypadKeysFor()` 给出三套按键定义（方向 3×3：四角功能键 + 中心回车 / 数字 4×4：右列四则运算 / 符号 6×5：标准键盘其余常用符号），`KeypadColsFor()` 给出各面板列数、行数按按键数推导，`KeypadComputeLayout()` 按字号实测文字宽度算圆角窗口与按键矩形（GDI 双缓冲绘制，悬停高亮）；命中用矩形判定，按下后位移超过阈值即改为拖拽（`SetCapture` + `Gui.Move`，只移动面板），抬起时要求按下与抬起落在同一按键才发送；发送前复用径向菜单的「前台窗口校验 + 物理修饰键释放等待」；面板同样带 `WS_EX_NOACTIVATE` 不抢焦点。面板状态放在 `Map()` 注册表 `keypadPanels` 里（三块面板可同时打开、互不干扰），鼠标钩子按引用计数注册/注销、按窗口句柄分发。
- **浮层公共层**：径向菜单与三块小键盘四者**互相独立**（任何关闭都只关自己，可同时显示），共用一个 `Esc`：`OverlayPush/Remove/Touch/Index` 维护"最近操作过的排在末尾"的栈，`OverlayOnEscape()` 关闭栈末尾那个；打开、鼠标移入、点击（含点空位/无效点击）都会把对应浮层置顶，全部关完后 `Esc` 立即归还系统。
