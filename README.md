...
**需求分两种情况**（模型先分清是哪一种，程序再按对应规则校验）：

| 情况 | 允许的动作 | 说明 |
|---|---|---|
| **一、输出文字（纯文本）** | `paste: 文本` | 要输出的文字，**推荐用它**：任何符号都会被原样粘贴，最可靠；不需要预先配置 |
| **一、输出文字（纯文本）** | `send: 文本` | 同上，但要输出的文字里**不能含 `^` `!` `+` `#` `{` `}`**（含了就当按键处理）；不需要预先配置 |
| **二、执行动作（按键 / 热键 / 键盘操作 / 程序 / 自身功能）** | 按键 / 组合键原文（如 `^c`、`{Enter}`） | **必须与您配置里某一项完全一样**，否则丢弃 |
| **二、执行动作** | `run: 命令行` / `self: 命令` | 启动程序 / 调用本软件功能，**必须与配置里某一项完全一样**，否则丢弃 |
| **二、执行动作** | `item: 名称` | 按名字执行您配好的那一项（最稳，模型优先用这个） |

- **纯文本输出照原样执行**：您说"输入：你好世界"，模型会输出 `paste: 你好世界`（或 `send: 你好世界`），程序直接把这段文字打给目标窗口——**不需要先在 `config.ini` 里建项**，也不需要模型改写或翻译。文字里含 `^ ! + # { }` 等符号时必须用 `paste:`。
...
- 执行期间显示"正在执行第 i/N 条"，**按 `Esc` 可立即中止剩余动作**；每条动作都会写进 `debug.log`（可审计）。
- **输出文字可以自由，执行动作必须命中配置（重要）**：
  · **输出文字**（`paste:` / `send:`）由模型按您的原话直接给出，**不需要预先配置**，程序照原样输出——这是"纯文本输出"这一类需求；
